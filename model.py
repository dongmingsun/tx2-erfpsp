...
def Decoder(x,numclasses,shape=[480,640],name='decoder',is_training=False,l2=None,bn=True,reuse=None):
    ...
    with tf.variable_scope(name):
        ...
        with arg_scope(get_conv_arg_scope(reg=reg, is_training=is_training,reuse=reuse)):
            ...
            net=slim.conv2d(net,numclasses,kernel_size=1,normalizer_fn=None,activation_fn=None,scope='conv5')
...
def train(x,l2,shape=[480,640],numclasses=66,reuse=None):
    x=Encoder(x, is_training=True,bn=True,l2=l2,reuse=reuse)
    x=Decoder(x,numclasses,shape=shape,name='decoder',is_training=True,l2=l2,bn=True,reuse=reuse)
    return x